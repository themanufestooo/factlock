...
2. **FactLock countersignature** — against the FactLock key resolved by `key_id`
...
4. **Lifecycle status** — only `ACTIVE`/`CLEARED` are trusted; `CORRECTED` is
   superseded and requires the replacement. `DISPUTED` is untrusted; `SUSPENDED` and
...
```json
{
  ...
  "inclusion_ok": true,
  "expired": false,
  "key_ids": { "business": "key_...", "factlock": "key_..." },
  ...
  "message": "VERIFIED — fla_...: signatures valid, in transparency log, status ACTIVE, 3d old."
}
```
